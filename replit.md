...
The backend is an Express.js application on Node.js. Real-time communication is handled via WebSockets using the `ws` library, specifically for audio streaming. The API design includes endpoints for session initialization and retrieval. The audio processing flow involves client-side audio capture and downsampling to 16kHz LINEAR16 PCM, streaming to the backend via WebSocket, forwarding to Speechmatics Real-Time API, and real-time transcription results sent back to the client. The final transcript is saved to a PostgreSQL database.
...
- **Real-time Transcription**: Converts spoken Arabic to text using Speechmatics Real-Time API.
...
- **Long Session Support**: Automatic stream restart every 4 minutes for reliability. Supports sessions of 1-2 hours or longer without data loss.

## External Dependencies

-   **Speechmatics Real-Time API** (`@speechmatics/real-time-client`, `@speechmatics/auth`): Primary speech recognition service for Arabic transcription with enhanced operating point for better accuracy. Uses LINEAR16 PCM encoding at 16kHz. Requires `SPEECHMATICS_API_KEY` environment variable.
